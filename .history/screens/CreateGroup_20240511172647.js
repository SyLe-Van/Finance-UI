...
export default function CreateGroup() {
  const [member, setMembers] = useState(["Member 1", "Member 2", "Member 3"]);
  const [pressedIndexes, setPressedIndexes] = useState([]);
  const [memberInputs, setMemberInputs] = useState(
    Array(member.length).fill("")
  );
  ...
  const addMember = () => {
    const newMember = `Member ${member.length + 1}`;
  ...
  const handlerSubmitGroup = () => {
    if (groupName === "") {
      ...
      // const newGroup = { groupName, memberNames: memberInputs };
      // setGroup([...group, newGroup]);
      // setGroupName("");
      // setMemberInputs(Array(members.length).fill(""));
      const objectGroup = {
        userId: id,
