...
export default function Result() {
  ...
  const formatNumberWithDot = (number) => {
    ...
    parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, "."); // Thêm dấu chấm phân tách hàng nghìn
    parts[1] = parts[1].replace(/\.?0*$/, ""); // Loại bỏ các chữ số 0 sau dấu thập phân nếu có
    return parts.join(".").replace(/\.$/, ""); // Loại bỏ dấu chấm cuối cùng nếu có
  };
