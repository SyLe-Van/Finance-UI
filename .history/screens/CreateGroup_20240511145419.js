...
export default function CreateGroup() {
  ...
  const handlerSubmitGroup = () => {
    if (groupName === "") {
      ...
      const newGroup = { groupName, memberNames: memberInputs }; // Thay đổi memberNames thành memberInputs
      setGroup([...group, newGroup]);
      setGroupName("");
      setMemberInputs(Array(members.length).fill("")); // Reset giá trị của memberInputs

      const objectGroup = {
        userId: id,
        name_group: groupName,
        member: memberInputs, // Thay đổi memberNames thành memberInputs
      };
      axios
        .post(
