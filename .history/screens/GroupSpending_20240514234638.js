...
export default function GroupSpending({ route }) {
  const {
    fromScreen,
    groupId,
    spendingItems: routeSpendingItems,
    nameGroup: routeNameGroup,
  ...
  useEffect(() => {
    if (fromScreen === "CreateGroup") {
      setButtonTitle("Save");
      setSpendingItems([
        { id: 0, name: "Spending 1" },
        { id: 1, name: "Spending 2" },
      ]);
    } else if (fromScreen === "CalculateSpending") {
      setButtonTitle("Update");
      setSpendingItems(routeSpendingItems);
      setNameGroup(routeNameGroup);
      setMembers(routeMembers);
    }
  }, [fromScreen, routeSpendingItems, routeNameGroup, routeMembers]);
  ...
  useEffect(() => {
    console.log("SpendingItems updated:", spendingItems);
  }, [spendingItems]);
    ...
    >
      <View style={styles.spendingInfoWrapper}>
        <SpendingInfo
          key={item.id}
          members={members}
          selectedMember={item.selectedMember || routeSelectedMember}
          value={item.value || routeValue}
          note={item.note || routeNote}
          onValueChange={(text) => {
            const updatedItems = [...spendingItems];
            updatedItems[index] = { ...updatedItems[index], value: text };
            setSpendingItems(updatedItems);
          }}
          onNoteChange={(text) => {
            const updatedItems = [...spendingItems];
            updatedItems[index] = { ...updatedItems[index], note: text };
            setSpendingItems(updatedItems);
          }}
          onMemberChange={(selectedMember) => {
            const updatedItems = [...spendingItems];
            updatedItems[index] = { ...updatedItems[index], selectedMember };
            setSpendingItems(updatedItems);
          }}
        />
      </View>
  ...
  const handleButtonPress = () => {
    if (fromScreen === "CreateGroup") {
      ...
      axios
        .put(
          `https://finance-api-kgh1.onrender.com/api/addPayList/${groupId}`,
          {
