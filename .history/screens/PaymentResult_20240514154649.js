    ...
    axios
      ...
      .then((response) => {
        const data = response.data.recommendations.map((payment) => ({
          ...payment,
          money_pay: parseFloat(payment.money_pay.toFixed(3)), // Làm tròn và chỉ lấy 3 chữ số sau dấu thập phân
        }));
        setPayments(data);
        setTotalSpending(parseFloat(response.data.total_payment.toFixed(3))); // Làm tròn và chỉ lấy 3 chữ số sau dấu thập phân
        setAverageSpending(parseFloat(response.data.average.toFixed(3))); //
      });
  }, []);
  return (
    <LinearGradient
