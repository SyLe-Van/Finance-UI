...
export default function GroupSpending() {
  ...
  const { groupName, members, groupId, memberIds } = route.params;
  ...
  const renderSpendingInfo = ({ item, index }) => (
    ...
    >
      <View style={styles.spendingInfoWrapper}>
        <SpendingInfo
          key={item.id}
          cost={costInputs[index] || ""} // Kiểm tra costInputs[index] trước khi truy cập
  ...
  useEffect(() => {
    setCostInputs(Array.from({ length: spendingList.length }, () => ""));
    setNotesInputs(Array.from({ length: spendingList.length }, () => ""));
  }, [spendingList]);

  const savePayListHandler = () => {
    const memberIdNamePairs = spendingList.map((item, index) => ({
      id: item.id,
      memberName: members[index],
    }));
    console.log("Member ID and Name pairs:", memberIdNamePairs);
  };
