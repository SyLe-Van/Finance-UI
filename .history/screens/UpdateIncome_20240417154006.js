...
const UpdateIncome = ({ navigation, route }) => {
  ...
  const fetchIncomeDetails = async (itemId) => {
    try {
      ...
      setNumberIncome(incomeDetails.value.toString());
      setText(incomeDetails.note);
      setSelectedCategory(incomeDetails.categoriesIncome);
      setDay(incomeDetails.date);
      setSelectedDate(new Date(incomeDetails.date));
