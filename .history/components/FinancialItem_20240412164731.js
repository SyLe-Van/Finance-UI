...
export default function FinancialItem({ type, item }) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      const userResponse = await axios.get(
        `https://finance-api-kgh1.onrender.com/api/getUser/${id}`
      );
      const userId = userResponse.data.data._id;
      console.log(userId);
      // Fetch expense ID
      const expenseResponse = await axios.get(
        `https://finance-api-kgh1.onrender.com/api/getExpense/${userId}/${item.expensesId}`
      );
      const expenseId = expenseResponse.data.data.expensesId;
      console.log(expenseId);
      // Delete expense
      await axios.delete(
        `https://finance-api-kgh1.onrender.com/api/deleteExpenses/${userId}/${expenseId}`
      );
  ...
  //     axios
  //       .get(
  //         `https://finance-api-kgh1.onrender.com/api/getExpense/:userId/:expensesIds/${id}`,
  //         {
  //           headers: {
  //             "Content-Type": "application/json",
  //           },
  ...
  //       )
  //       .then((response) => {
  //         const expenseId = response.data.data.setExpenses();
  //       })
  //       .catch((error) => {
  //         console.log(error);
  //         // hien thi o da
