...
export default function SpendingInfo({ members, memberIds }) {
  ...
  const getMemberIdByName = (memberName) => {
    const index = members.indexOf(memberName);
    return index !== -1 ? memberIds[index] : "";
  };
      ...
      <View style={styles.name_cost}>
        <Dropdown
          ...
          onChange={(item) => {
            ...
            setSelectedMemberId(getMemberIdByName(item.label));
          }}
        />
