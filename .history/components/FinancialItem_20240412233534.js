...
export default function FinancialItem({ type, item }) {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        const expenseId = response.data._id; // Assuming the expense ID is in the response data
        console.log("Expense ID:", expenseId);
        setExpenseId(expenseId); // Set the expenseId state variable
      })
      .catch((error) => console.log(error));
  }, [userId, item.expensesId]);
  return (
    <TouchableHighlight
