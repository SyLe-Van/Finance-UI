...
export default function SpendingInfo({ members, memberIds, onChangeText }) {
  ...
  const handleMemberChange = (selectedMemberLabel) => {
    setSelectedCategory(selectedMemberLabel);
    const selectedMemberIndex = members.indexOf(selectedMemberLabel);
    if (selectedMemberIndex !== -1) {
      const selectedMemberId = memberIds[selectedMemberIndex];
      onMemberChange(selectedMemberId); // Gọi lại hàm callback và truyền memberId
    }
  };
  return (
        ...
        <Dropdown
          ...
          onChange={(item) => {
            if (item && item.label) {
              ...
              if (selectedMemberIndex !== -1) {
                onChangeText(memberIds[selectedMemberIndex]);
              }
            }
          }}
        />
