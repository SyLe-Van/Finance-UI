...
export default function FinancialItem({ type, item }) {
  ...
  async function handleDelete() {
    try {
      const response = await axios.delete(
        `https://finance-api-kgh1.onrender.com/api/deleteExpenses/${userId}/${item._id}`
      );
  ...
  //   useEffect(() => {
  //     async function fetchExpenseId() {
  //       try {
  //         const response = await axios.get(
  //           `https://finance-api-kgh1.onrender.com/api/getExpense/${userId}/${item._id}`,
  //           {
  //             headers: {
  //               "Content-Type": "application/json",
  //             },
