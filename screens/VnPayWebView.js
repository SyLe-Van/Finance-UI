...
const VnPayWebView = ({ navigation }) => {
  const { isPremium, setIsPremium, id, updateData, setUpdateData } =
    useContext(AuthContext);
  const randomOrderId = generateRandomNumberWithLength(10);
  const paymentUrl = `http://188.166.230.18:8888/order/create_payment_url?amount=300000&orderId=${randomOrderId}`;
